...
function buildUrl(baseUrl: string, cats: string[], filters: string[]): string {
  const params = [
    ...cats.map((c) => `cat=${encodeURIComponent(c)}`),
    ...filters.map((f) => `filter=${encodeURIComponent(f)}`),
  ];
  return params.length ? `${baseUrl}?${params.join("&")}` : baseUrl;
}

export function toggleCatUrl(slug: string, activeCats: string[], activeFilters: string[], baseUrl: string) {
  const next = activeCats.includes(slug)
    ? activeCats.filter((c) => c !== slug)
    : [...activeCats, slug];
  return buildUrl(baseUrl, next, activeFilters);
}

export function toggleFilterUrl(opt: string, activeCats: string[], activeFilters: string[], baseUrl: string) {
  const next = activeFilters.includes(opt)
    ? activeFilters.filter((f) => f !== opt)
    : [...activeFilters, opt];
  return buildUrl(baseUrl, activeCats, next);
}

export function resetUrl(baseUrl: string) {
  return baseUrl;
}
...
export default function FilterSidebarContent({
  ...
  const hasAny = activeCats.length > 0 || activeFilters.length > 0;
      ...
      <div style={{ borderTop: "1px solid #e7e4df" }} className="py-5">
        ...
        {categories.map((cat) => {
          ...
          return (
            <Link
              key={cat.id}
              href={toggleCatUrl(cat.slug, activeCats, activeFilters, baseUrl)}
        ...
        <div key={label} style={{ borderTop: "1px solid #e7e4df" }} className="py-5">
          ...
          {options.map((opt) => {
            ...
            return (
              <Link
                key={opt}
                href={toggleFilterUrl(opt, activeCats, activeFilters, baseUrl)}
                className="flex items-center gap-2.5 py-1.5"
              >
                <span
                  className="inline-block shrink-0"
